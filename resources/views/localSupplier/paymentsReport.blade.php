      ...
      <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="card card-primary">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h3 class="card-title">Supplier Payments Report</h3>
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-12">
                <form method="get" action="{{route('paymentsReport') }}">
              ...
              <div class="col-md-12" id="printReport">
                <center><h5 style="margin: 0px">Other Receives</h5></center>
                <div class="table-responsive">
                  @if(!empty($start_date) && !empty($end_date))
                    <center><h6 style="margin: 0px">From : {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} To : {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}</h6></center>
                  @else
                  ...
                  <table class="reportTable" style="width: 100%; font-size: 12px;" cellspacing="0" cellpadding="0">
                    <thead> 
                      <tr style="background: #ccc; color: #000"> 
                        ...
                        <th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">Name</th>
                        <th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">Phone</th>
                        <th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">Address</th>
                        <th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">Note</th>
                        <th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">Transaction Method</th>
                        <th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">Amount</th>
                      </tr>
                    </thead>
                    <tbody> 
                      @foreach($localSuppliers as $key => $data)
                      <tr> 
                        ...
                        <td style="border: 1px solid #ddd; padding: 3px 3px"><a href="{{route('localSuppliers.show', $data->local_supplier_id)}}">{{$data->localSupplier->name ?? ''}}</a></td>
                    ...
                    <tfoot> 
                      <tr> 
                        ...
                        <td style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px"><b>{{number_format($localSuppliers->sum('amount'), 2)}}</b></td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
          ...
          <div class="card-footer" >
            <div class="col-md-12" style="display:flex; justify-content: end">
              {{$localSuppliers->appends(Request::except('page'))->links()}}
            </div>
          </div>
        </div>
        <!-- /.box -->
